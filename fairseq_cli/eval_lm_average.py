...
from fairseq.checkpoint_utils import save_state
...
def evaluate(models, sample, device, gen_timer, scorer, bpe_toks, output_word_probs, output_word_stats, remove_bos_token=False, ensemble_type="cached_prior", prior=None):
    ...
    hypos = scorer.generate(models, sample, ensemble=False, ensemble_weighted_average=None, ensemble_average=False, prior=None,all_reduce=torch.distributed.is_initialized())
...
def average_soup(soup, weights=None, uniform=True, topk=-1):
    # if avg:
    res = deepcopy(soup[0])
    soup = [x.state_dict() for x in soup]
    averaged_soup = average(soup, weights=weights, uniform=uniform, topk=topk)
    res.load_state_dict(averaged_soup)
    return res
    # else:
...
def argmax_soup(soup, weights):
    res = deepcopy(soup[0])
    soup = [x.state_dict() for x in soup]
    averaged_soup = soup[int(np.argmax(weights))].copy()
    res.load_state_dict(averaged_soup)
    return res
...
def dynamic_eval_lm(
    ...
    if ensemble_type == 'updating_prior':
        precomputed_prior = None
        prior = None
    pbar = tqdm(enumerate(dev_batch_iterator))
    for ix, sample in pbar:
        ...
        pbar.set_description(f"ppl: {target_ppl.item()}")
...
def main(cfg: DictConfig, **unused_kwargs):
    ...
    # weights = "0.0000003839,0.7321925875,0.2636055121,0.0000064328,0.0000042374,0.0027307478,0.0000012282,0.0014588739"
    # weights = "0.0000119093,0.3188265481,0.1768292506,0.4563316552,0.0002717303,0.0000990119,0.0000518462,0.0475780631"
    # weights = "0.0000004435,0.1226956893,0.0035702,0.0000141227,0.8719714219,0.0003292498,0.0013598519,0.0000590209"
    # weights = "0.0,0.5559796966,0.3963835894,0.0036813877,0.0000000046,0.0437960744,0.0000000066,0.0001592465"
    # weights = "0.0000533368,0.0222241124,0.015414305,0.002345576,0.0324713774,0.0006956184,0.0009971368,0.9257985444"
    # weights = "0.0000000038,0.1607861375,0.1865282211,0.3018310914,0.0419556143,0.0167839876,0.0024129786,0.2897019772"
    #weights = "0.0000005583,0.793581613,0.1905674287,0.002584057,0.0001811502,0.0012247967,0.0000031676,0.0118572587"
    weights = precomputed_prior 
    try:
        random_idx = np.random.choice(range(len(models)), p=weights)
    except:
   ...
   #assert sum(weights) == 1.0
    ...
    if cfg.common_eval.posterior_average:
        soup = models 
        best_average = average_soup(soup, uniform=False,weights=weights)
        dev_itr = task.eval_lm_dataloader(
                dataset=eval_dataset,
                max_tokens=cfg.dataset.max_tokens or 36000,
                batch_size=cfg.dataset.batch_size,
    ...
    elif cfg.common_eval.argmax_expert:
    
        best_average = models[int(np.argmax(weights))]
        dev_itr = task.eval_lm_dataloader(
                dataset=eval_dataset,
                max_tokens=cfg.dataset.max_tokens or 36000,
                batch_size=cfg.dataset.batch_size,
    ...
    elif cfg.common_eval.uniform_average:
        soup = models 
        best_average = average_soup(soup, uniform=True,weights=None)
        dev_itr = task.eval_lm_dataloader(
                dataset=eval_dataset,
                max_tokens=cfg.dataset.max_tokens or 36000,
                batch_size=cfg.dataset.batch_size,
        ...
        # trainer=trainer,
            ...
            precomputed_prior=precomputed_prior,
            ensemble_type=cfg.common_eval.ensemble_type)
    elif cfg.common_eval.greedy_soup:

        best_ppl = np.inf 
        pbar = trange(100)
        soup = [average_soup(models, uniform=False, weights=weights)]
        dev_itr = task.eval_lm_dataloader(
                dataset=eval_dataset,
                max_tokens=cfg.dataset.max_tokens or 36000,
                batch_size=cfg.dataset.batch_size,
        ...
        for _ in pbar:
            if patience > 10:
                break
            random_idx = np.random.choice(range(len(models)), p=weights)
            random_expert = models[random_idx]
            random_expert_path = model_[random_idx]

            candidate_model = average_soup(soup + [random_expert.cpu()], uniform=True)
            dev_itr = task.eval_lm_dataloader(
                dataset=eval_dataset,
                max_tokens=cfg.dataset.max_tokens or 36000,
                batch_size=cfg.dataset.batch_size,
            ...
            if results[-1] < best_ppl:
                soup[0] = candidate_model.cpu()
                soup_size += 1
                soup_paths.append(random_expert_path)
                best_average = candidate_model
                patience = 0
                best_ppl = results[-1]
            patience += 1
            pbar.set_description(f"best ppl: {best_ppl}, len of soup: {soup_size}")
    ...
    with open(cfg.common_eval.results_path, 'w+') as f:
        res = {"ppl": results[-1]}
        if cfg.common_eval.greedy_soup:
            res['soup_size'] = soup_size
            res['soup'] = soup_paths 
        json.dump(res, f)

    #tmp_model = torch.load(cfg.common_eval.path.split(':')[0])
    #tmp_model['model'] = dict(best_average.state_dict())
    from pathlib import Path
    if not Path(cfg.common_eval.best_average_output_dir).is_dir():
        Path(cfg.common_eval.best_average_output_dir).mkdir(parents=True,exist_ok=True)
    #torch.save(tmp_model, Path(cfg.common_eval.best_average_output_dir) /"checkpoint_last.pt")
    #if results[-1] > best_ppl:
