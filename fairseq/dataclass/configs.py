...
class DatasetConfig(FairseqDataclass):
    ...
    target_eval: str = field(
        default="test",
        metadata={"help": "data subset to generate (train, valid, test)"},
    )

    undomain_eval: str = field(
        default="test",
        metadata={"help": "data subset to generate (train, valid, test)"},
    )

    undomain_replay: str = field(
        default="test",
        metadata={"help": "data subset to generate (train, valid, test)"},
    )
...
class CheckpointConfig(FairseqDataclass):
    ...
    symlink_best_and_last_checkpoints: bool = field(
        default=False,
        metadata={"help": "Symlink best and last checkpoints instead of copying"},
    )
...
class CommonEvalConfig(FairseqDataclass):
    ...
    eval_topk: Optional[int] = field(
        default=None, metadata={"help": "number of experts to include"}
    )

    max_samples: Optional[int] = field(
        default=None, metadata={"help": "maximum number of samples to compute perplexity on"}
    )
    ...
    partial_load: bool = field(
        default=False,
        metadata={"help": "if set, partially load desynchronized checkpoint"},
    )

    add_adapters: Optional[int] = field(
        default=None, metadata={"help": "if set, will add adapters and fine-tune for dynamic eval"}
    )


    add_domain_token: bool = field(
        default=False,
        metadata={"help": "if set, add domain token"},
    )



    use_expert: Optional[int] = field(
        default=None, metadata={"help": "use expert"}
    )

    replay: bool = field(
        default=False, metadata={"help": "replay undomain data at regular intervals"}
    )
...
@dataclass
class DynamicEvalLMConfig(FairseqDataclass):
    output_word_probs: bool = field(
        default=False,
        metadata={
            "help": "if set, outputs words and their predicted log probabilities to standard output"
...
class FairseqConfig(FairseqDataclass):
    ...
    dynamic_eval_lm: DynamicEvalLMConfig = DynamicEvalLMConfig()
