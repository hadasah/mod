...
class Trainer(object):
    ...
    def save_checkpoint(self, filename, extra_state, training_finished=False,):
        """Save all training state in a checkpoint file."""
        # only save one checkpoint unless we have different experts per DDP worker
        if self.is_data_parallel_master or self.cfg.model.moe_freq > 0 or getattr(self.cfg.model, "desynchronize", False):
    ...
    def load_checkpoint(
        ...
        shared_filename = re.sub('rank-[0-9]+', 'shared', filename)
        shared_exists = filename != shared_filename and PathManager.isfile(shared_filename)
        if bexists:
            load_on_all_ranks = (
            ...
            if (
                ...
                shared_state = checkpoint_utils.load_checkpoint_to_cpu(
                    shared_filename, 
                    load_on_all_ranks=load_on_all_ranks, 
                    moe_freq=getattr(self.cfg.model, "moe_freq", 0),
                    desynchronize=getattr(self.cfg.model, "desynchronize", False)
                )
                shared_last_optim_state = shared_state.get("last_optimizer_state", None)

                state['model'].update(shared_state['model'])
                last_optim_state['state'].update(shared_last_optim_state['state'])
    ...
    def get_train_iterator(
        ...
        if torch.distributed.is_initialized() and shard_batch_itr and not getattr(self.cfg.task, 'unbalanced', None):
            num_shards = torch.distributed.get_world_size() // 8
            shard_id = torch.distributed.get_rank() % (torch.distributed.get_world_size() // 8)
        elif torch.distributed.is_initialized() and shard_batch_itr and getattr(self.cfg.task, 'unbalanced', None):
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        # logger.info(f"[{self.get_num_updates()}] before fwd")
