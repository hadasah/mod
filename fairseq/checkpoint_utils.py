...
def save_checkpoint(cfg: CheckpointConfig, trainer, epoch_itr, val_loss, training_finished=False):
    ...
    if not trainer.is_data_parallel_master and \
        (getattr(trainer.cfg.model, "moe_freq", 0) == 0 and not getattr(trainer.cfg.model, "desynchronize", False) or getattr(trainer.cfg.model, "alternate_decoder_ffn_embed_dim", 0) > 0) :
        return
    ...
    if val_loss is not None and cfg.keep_best_checkpoints > 0 and not cfg.no_best_checkpoints:
        checkpoint_conds[
    ...
    if len(checkpoints) > 0:
        trainer.save_checkpoint(checkpoints[0], extra_state, training_finished=training_finished)
        for cp in checkpoints[1:]:
            srcs = [checkpoints[0]]
            dests = [cp]
            if (getattr(trainer.cfg.model, "moe_freq", 0) > 0 or getattr(trainer.cfg.model, "desynchronize", False)) and (
                not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0 
            ):
                srcs.append(re.sub('rank-[0-9]+', 'shared', checkpoints[0]))
                dests.append(re.sub('rank-[0-9]+', 'shared', cp))
    ...
    if not end_of_epoch and cfg.keep_interval_updates > 0:
        suffixes = [suffix]
        if (
            getattr(trainer.cfg.model, "moe_freq", 0) > 0 
...
def load_checkpoint_to_cpu(path, arg_overrides=None, load_on_all_ranks=False, moe_freq=0, desynchronize=False, partial_load=False):
    ...
    if moe_freq > 0 or desynchronize or partial_load:
        with open(local_path, "rb") as f:
            expert_state = torch.load(f, map_location=torch.device("cpu"))
        with open(re.sub('rank-[0-9]+', 'shared', local_path), "rb") as f:
...
def load_model_ensemble_and_task(
    ...
    for filename in filenames:
        ...
        for shard_idx in range(num_shards):
            ...
            if state is None:
                state = load_checkpoint_to_cpu(filename, arg_overrides, moe_freq=moe_freq, desynchronize=desynchronize, partial_load=partial_load)
            ...
            keys_chk = set(state.keys())
            keys_model = set([name for name, param in model.named_parameters()])
    ...
    for filename, model_state_dict, optimizer_state_dict in model_save_list:
        ...
        if isinstance(cfg, DictConfig):
            no_save_optimizer_state = (
                cfg.checkpoint.no_save_optimizer_state
                or (
                    cfg.checkpoint.no_save_optimizer_state_on_training_finished
                    and training_finished
                )
            )
        else:
            no_save_optimizer_state = (
                cfg.no_save_optimizer_state
                or (
                    cfg.no_save_optimizer_state_on_training_finished 
                    and training_finished
                )
            )
