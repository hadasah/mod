...
def extract(input_df, output_file, model_path, batch_size, submitit=False):
    ...
    for batch_json in tqdm(df_iterator,
        ...
        with torch.no_grad():
            output = model(**input_ids,labels=input_ids['input_ids'].clone())
            loss = output[0]
            print(torch.exp(loss))
            #vs = []
            # for ix, idx in enumerate(last_non_masked_idx):
                # vs.append(out[0][ix, :idx, :])
            ppls.append(torch.unsqueeze(torch.exp(loss), 0))
...
#                vs.append(torch.mean(vectors_[0][ix, :idx, :], 0).unsqueeze(0)) # Models outputs are now tuples
        ...
        #ids.append(indices)
    ppl = {"ppL": torch.cat(ppls, 0).cpu().mean().item()}
    #vectors_ = torch.cat(vectors, 0).cpu()
    with open(output_file, 'w+') as f:
        json.dump(ppl, f)

    #h.save(ppls.mean(), output_file)
